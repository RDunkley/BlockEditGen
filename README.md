...
* type - Type of data the value represents. This determines what UI control represents the value. The following types are supported:
  ...
  * array - Represents an array of unsigned integers. The subtype specifies the element type ('byte', 'ushort', 'uint', or 'ulong'). Size must be byte-aligned and a multiple of the element size. Values are displayed in a hex editor; read-only values cannot be edited. The hex digits are grouped by element so it is clear where one value ends and the next begins, giving two nibbles per group for 'byte', four for 'ushort', eight for 'uint', and sixteen for 'ulong'. The hex editor reflows to the width of the window, so the number of bytes on a row and the number of rows change as it is resized. A character column is shown next to the hex for the element types where one is meaningful: 'byte' arrays get an ASCII column with one character per byte, and 'uint' arrays get a UTF-32 column where each 32-bit element is read little endian as one Unicode code point. Anything that is not printable is shown as a period. Note that this is not the same as the 'Unicode' string subtype, which is UTF-16.
